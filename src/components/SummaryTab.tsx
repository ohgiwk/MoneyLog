  ...
  return (
    <div>
        ...
        <div className="px-4 pt-3">
          <div className="flex rounded-xl bg-slate-100 p-1">
            {(["overview", "fixed", "goals"] as SubPage[]).map((page) => {
              ...
              return (
                <button key={page} onClick={() => setSub(page)}
      ...
      <div className="p-4 space-y-4">
        {sub === "overview" && <Overview transactions={transactions} month={month} fixedExpenses={fixedExpenses} />}
        {sub === "fixed" && (
          <FixedExpenseList
            userId={userId}
...
function Overview({ transactions, month, fixedExpenses }: {
  ...
  const routineExpense = monthTx.filter((t) => t.type === "expense" && t.expense_kind === "routine").reduce((s, t) => s + t.amount, 0);
  const oneTimeExpense = monthTx.filter((t) => t.type === "expense" && t.expense_kind === "one_time").reduce((s, t) => s + t.amount, 0);
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-2.5">
        ...
        <Row label="ルーチン費" value={`-${formatYen(routineExpense)}`} valueColor="text-rose-500" />
        <Row label="臨時費" value={`-${formatYen(oneTimeExpense)}`} valueColor="text-amber-500" />
        <div className="h-px bg-slate-100" />
        <Row label="収支" value={(balance >= 0 ? "+" : "") + formatYen(balance)}
          valueColor={balance >= 0 ? "text-emerald-600" : "text-rose-500"} bold />
      </div>
      ...
      {totalSaved > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <div className="text-2xl font-bold text-emerald-600 mb-1">-{formatYen(totalSaved)}<span className="text-sm font-normal text-slate-400">/月</span></div>
...
function FixedExpenseList({ userId, fixedExpenses, reload, onEditingChange }: {
  userId: string;
  fixedExpenses: FixedExpense[];
  reload: () => void;
  ...
  useEffect(() => {
    if (fixedExpenses.length === 0 && !seeding) {
      ...
      supabase.from("fixed_expenses").insert(rows).then(() => {
        reload();
        setSeeding(false);
      });
    }
  ...
  const totalBaseline = activeExpenses.filter((f) => f.baseline_amount > 0).reduce((s, f) => s + f.baseline_amount, 0);
  const totalCurrent = activeExpenses.filter((f) => f.baseline_amount > 0).reduce((s, f) => s + (f.amount ?? 0), 0);
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        ...
        <Row label="固定費合計（月額）" value={formatYen(totalAmount)} valueColor="text-slate-700" />
        {totalSaved > 0 && (
          <>
            <div className="mt-2" />
            <Row label="初回登録時との差" value={`-${formatYen(totalSaved)}/月`} valueColor="text-emerald-600" bold />
            <div className="text-xs text-slate-400 mt-1">年間換算 -{formatYen(totalSaved * 12)}</div>
          </>
        )}
      </div>
        ...
        : (
          <div className="bg-white rounded-2xl shadow-sm divide-y divide-slate-50">
            {filtered.length === 0
              ...
              : filtered.map((f) => (
                <div key={f.id} className="flex items-center px-4 py-3 gap-3 active:bg-slate-50 cursor-pointer"
...
function FixedExpenseForm({ userId, expense, onClose }: {
  userId?: string;
  ...
  async function save() {
    ...
    setSaving(true);
    if (expense) {
      await supabase.from("fixed_expenses").update({ name, category, amount: amt, cycle, status, notes: notes || null }).eq("id", expense.id);
    } else {
      await supabase.from("fixed_expenses").insert({
        user_id: userId!,
        name, category, amount: amt,
        baseline_amount: amt,
        cycle, status, notes: notes || null,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-semibold text-slate-700">{expense ? "固定費を編集" : "固定費を追加"}</span>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
        ...
        <div>
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="例: Netflix"
          ...
          <div className="grid grid-cols-4 gap-2 mt-1">
            {FIXED_EXPENSE_CATEGORIES.map((c) => (
              <button key={c.name} type="button" onClick={() => setCategory(c.name)}
                className={"flex flex-col items-center py-2 rounded-xl text-xs gap-1 border " +
                  (category === c.name ? "border-emerald-400 bg-emerald-50" : "border-slate-100 bg-slate-50")}>
                <span className="text-base">{c.icon}</span>
                <span className="text-[10px] text-slate-600 text-center leading-tight">{c.name}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select value={cycle} onChange={(e) => setCycle(e.target.value as FixedExpense["cycle"])}
              className="w-full mt-1 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
            ...
            {(["active", "reviewing", "cancelled"] as const).map((s) => (
              <button key={s} type="button" onClick={() => setStatus(s)}
                className={"flex-1 py-1.5 rounded-lg text-xs font-semibold border " +
                  (status === s ? `${STATUS_LABELS[s].color} border-current` : "border-slate-100 text-slate-400")}>
                {STATUS_LABELS[s].label}
              </button>
            ))}
          </div>
        ...
        <div className="flex gap-3 pt-2">
          {expense && (
            <button onClick={remove} className="px-4 py-2.5 rounded-xl border border-rose-200 text-rose-500 text-sm font-semibold">
              削除
            </button>
          )}
