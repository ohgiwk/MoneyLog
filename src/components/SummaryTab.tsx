    ...
    <div>
      ...
      <div className="px-4 pt-3">
        <div className="flex rounded-xl bg-slate-100 p-1">
          {(['overview', 'detail'] as SubPage[]).map((page) => {
            const labels = { overview: '概要', detail: '詳細' }
            return (
              <button
                key={page}
                onClick={() => setSub(page)}
...
function Overview({
  ...
  const totalBaseline = activeFixed.reduce((s, f) => s + f.baseline_amount / (f.cycle === 'yearly' ? 12 : 1), 0)
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-2.5">
        ...
        <Row label="固定費" value={`-${formatYen(Math.round(totalFixed))}`} valueColor="text-slate-500" />
      ...
      {totalSaved > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <div className="text-xs text-slate-400">累計節約 {formatYen(Math.round(totalSaved * 12))}/年換算</div>
        </div>
      )}
    </>
  )
