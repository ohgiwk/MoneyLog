    ...
    <div>
      ...
      <div className="p-4 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
          ...
          {type === "expense" && (
            <div className="flex rounded-xl bg-slate-100 p-1">
              <button type="button" onClick={() => setExpenseKind("routine")}
                className={"flex-1 py-1.5 rounded-lg text-xs font-semibold transition " +
                  (expenseKind === "routine" ? "bg-white shadow text-slate-700" : "text-slate-400")}>
                ルーチン費
              </button>
              <button type="button" onClick={() => setExpenseKind("one_time")}
                className={"flex-1 py-1.5 rounded-lg text-xs font-semibold transition " +
          ...
          <button type="submit" disabled={submitting}
            className={"w-full py-3 rounded-xl text-white font-semibold shadow disabled:opacity-50 " +
              (type === "expense" ? "bg-rose-500 active:bg-rose-600" : "bg-emerald-500 active:bg-emerald-600")}>
            記録する
          </button>
        </form>
                            ...
                            <div>
                              <div className="flex items-center gap-1.5">
                                ...
                                {t.expense_kind === "one_time" && (
                                  <span className="text-[10px] bg-amber-50 text-amber-600 px-1.5 py-0.5 rounded-full">臨時</span>
                                )}
                              </div>
